...
class UpdateProfileAction
{
    ...
    // public static function updateProfile(int $userId, array $inputs, EmailService|null $emailService = null): User
    // {
    //     $user = new User();
    //     $user->find($userId);
    ...
    //     $emailChanged = isset($inputs['email']) && $inputs['email'] !== $user->email;
    ...
    //         $user->user_status_id = $inputs['user_status_id'];
    //     }
    ...
    //         $token = Helper::generateSecureToken(64);

    //         $entity = new EmailVerificationToken();
    //         $entity->user_id = $user->getId();
    //         $entity->token_hash = hash('sha256', $token);
    //         $entity->expires_at = Loader::config()->expiry('email_verification')->format('Y-m-d H:i:s');
    //         $entity->insert();
    ...
    //             $user->email,
    //             'Email Verification',
    //             'Please verify your email using this link: ' . Flight::app()->request()->scheme . '://' . Flight::app()->request()->host . '/auth/email-verification?token=' . $token
    //         );
    //     }

    //     $user->save();

    //     AuditLogService::record('User profile updated', [
    //         'user_id' => $user->getId(),
